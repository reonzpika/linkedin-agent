...
- **linkedin-post-create** (skill): Only way to start a new post. Prompts for topic/URL and optional pillar/angle; creates session folder and `input.json`; runs `scripts/plan_from_url.py` → present plan in chat → after approval, runs `scripts/research.py` → present research → runs `scripts/scout.py` then, if more than 6 targets, `scripts/pick_targets.py` then `scripts/draft.py` → present draft in chat → after draft approval, offers optional image generation for carousel posts (Phase 4c: user must explicitly confirm before `scripts/generate_images.py` runs) → runs `scripts/assemble_session_state.py` then schedules via `tools/scheduler.schedule_execution_auto_slot`. Session state is written so `execute_post.py` can run later.
